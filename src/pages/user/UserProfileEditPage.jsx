...
export default function UserProfileEditPage () {
  ...
  useEffect(() => {
    if (!userData) return
    const isChanged =
      Number(personalData.age) !== userData?.age ||
      personalData.gender !== userData?.gender ||
      textAreaData !== userData?.bio
    setStatus(isChanged ? 'editing' : 'idle')
  }, [personalData, textAreaData])
          ...
          <div id='editSection' className='relative w-full flex flex-col gap-3'>
            ...
            >
              <div className='form-control w-full flex flex-col gap-y-1'>
                ...
                <select
                  className='select select-primary bg-transparent select-xs outline-0 text-[0.9em]'
                  name='genderInput'
                  id='genderInput'
              ...
              <div className='form-control w-full flex flex-col gap-y-1'>
                <label className='font-semibold capitalize' htmlFor='ageInput'>
                  age
                </label>
                <input
                  type='number'
