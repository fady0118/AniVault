    ...
    queries: [
      ...
      },
      {
        ...
        queryFn: async () => {
          ...
          const fav_anime_Arr =
            fav_Data?.data?.anime.map(item => ({
              anime: { path: 'anime', name: item.title, ...item }
            })) || []
          const fav_manga_Arr =
            fav_Data?.data?.manga.map(item => ({
              anime: { path: 'manga', name: item.title, ...item }
            })) || []
          const fav_character_Arr =
            fav_Data?.data?.characters.map(item => ({
              anime: { path: 'characters', ...item }
            })) || []
          const fav_people_Arr =
            fav_Data?.data?.people.map(item => ({
              anime: { path: 'people', ...item }
            })) || []
          return (
            {
              anime: fav_anime_Arr,
              manga: fav_manga_Arr,
              characters: fav_character_Arr,
              people: fav_people_Arr
            } || ''
          )
        }
      },
      ...
      <div className='flex flex-col py-2 gap-y-2'>
        <div className='w-full flex flex-row justify-between'>
          {Object.entries(heading).map((d, i) => (
            <div
              key={i}
              className='flex flex-row items-center gap-x-2 text-xs capitalize'
            >
              <p>{d[0]}:</p>
              <p>{d[1]}</p>
            </div>
        ...
        <div className='w-full flex flex-row items-center h-3 fill-to-right rounded-sm overflow-hidden'>
          {Object.values(data).map((d, i) => (
            <div
              key={i}
              style={{
                width: `${((100 * d) / total.total_entries).toFixed(1)}%`
              }}
        ...
        <div className='w-full grid grid-cols-2 gap-x-5'>
          <div className='w-full flex flex-col space-y-2'>
            {Object.entries(data).map((d, i) => (
              <div
                key={i}
                className='flex flex-row justify-between items-center gap-x-2 text-xs capitalize'
              >
                <div className='flex flex-row items-center gap-x-2'>
                  <p
                    className={`w-2 aspect-square rounded-full ${colors[i]}`}
                  ></p>
                  <p>{d[0]}:</p>
                </div>
                <p>{d[1]}</p>
      ...
      {userQ.isPending ? (
        ...
        <>
          <div className='relative left-1/2 -translate-x-1/2 z-10 w-[95%] flex flex-col space-y-3 pt-15 pb-3 text-dark-amethyst-smoke-50 dark:text-text-dark'>
            <div
              id='title'
              className='order-1 mt-3 min-w-1/2 w-fit rounded-md px-3 py-1 box-colors flex items-center space-x-2'
            >
              <div className='text-sm/relaxed sm:text-lg/relaxed font-bold dark:text-text-dark'>
                {userQ?.data.username}'s Profile
              </div>
              <Link
            ...
            <div
              id='userContent'
              className='relative order-2 w-full flex flex-col sm:flex-row sm:justify-start gap-3'
            >
              <div className='sm:sticky sm:top-18 flex flex-col gap-y-3 h-fit w-1/4 min-w-48 max-w-64 '>
                <div id='image' className='order-1'>
                  <img
                    ...
                    src={
                      userQ?.data?.images?.webp?.image_url ||
                      userQ?.data?.images?.jpg?.image_url
                    }
                    alt=''
                  />
                </div>
                    ...
                    <div id='information' className='w-full'>
                      ...
                      <div className='px-3 py-2 text-xs font-light'>
                        <div className='grid grid-cols-1 w-full gap-y-2.5 lg:text-[1.1em]'>
                          {renderInfoStr(
                            'last online',
                            `${DateTimeFormatter(userQ?.data?.last_online)}`
                          )}
                          {userQ?.data?.birthday
                            ? renderInfoStr(
                                'birthday',
                          ...
                          {userQ?.data?.location
                            ...
                            : ''}
                          {userQ?.data?.gender
                            ? renderInfoStr('gender', `${userQ?.data?.gender}`)
                            : ''}
                          {userQ?.data?.joined
                            ? renderInfoStr(
                                'joined',
                                `${DateTimeFormatter(userQ?.data?.joined)}`
                              )
                            : ''}
                        </div>
                      </div>
                      ...
                      <div id='friends' className='w-full'>
                        ...
                        <div className='grid grid-cols-4 p-2 gap-0.5'>
                          {friendsQ?.data.slice(0, 11).map((f, i) => (
                            <div
                              key={i}
                              className='w-full aspect-square hover:'
                            >
                              <Link to={`/user/${f?.user?.username}`}>
                                <img
                                  ...
                                  src={
                                    f?.user?.images.webp.image_url ||
                                    f?.user?.images.jpg.image_url
                                  }
                                  alt=''
                                />
                              </Link>
              ...
              <div className='w-3/4 grow flex flex-col gap-y-3'>
                <div
                  id='about'
                  className='w-fit max-w-full h-fit min-h-32 rounded-lg overflow-hidden box-colors'
                >
                ...
                <div
                  id='statistics'
                  className='w-full flex flex-col py-2 gap-y-2 rounded-lg overflow-hidden box-colors'
                >
                  <div className='border-b border-amethyst-smoke-700/40 dark:border-amethyst-smoke-500/40 pt-0.5 px-3 font-semibold text-md/relaxed capitalize'>
                    statistics
                  </div>
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 px-3'>
                    <div>
                      <div className='border-b border-amethyst-smoke-200/40 pt-0.5 font-semibold text-sm/relaxed capitalize'>
                        ِAnime stats
                      </div>
                      {renderStats(
                        {
                        ...
                        },
                        {
                          ...
                          'episodes watched':
                            userQ?.data?.statistics?.anime.episodes_watched
                        },
                        {
                          'days watched':
                            userQ?.data?.statistics?.anime.days_watched,
                          'mean score':
                            userQ?.data?.statistics?.anime.mean_score
                        }
                      )}
                    </div>
                    <div>
                      ...
                      {renderStats(
                        {
                          ...
                          'plan to read':
                            userQ?.data?.statistics?.manga.plan_to_read
                        },
                        {
                          total_entries:
                            userQ?.data?.statistics?.manga.total_entries,
                          reread: userQ?.data?.statistics?.manga.reread,
                          'chapters read':
                            userQ?.data?.statistics?.manga.chapters_read,
                          'volumes read':
                            userQ?.data?.statistics?.manga.volumes_read
                        },
                        {
                          'days read': userQ?.data?.statistics?.manga.days_read,
                          'mean score':
                            userQ?.data?.statistics?.manga.mean_score
                        }
                      )}
                    </div>
                  </div>
                </div>
                {favoritesQ?.data?.anime.length ||
                favoritesQ?.data?.manga.length ||
                favoritesQ?.data?.characters.length ||
                favoritesQ?.data?.people.length ? (
                  <div
                    id='favorites'
                    className='w-full flex flex-col py-2 gap-y-2 rounded-lg overflow-hidden box-colors'
                  >
