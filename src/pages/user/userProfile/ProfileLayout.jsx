...
export default function ProfileLayout () {
  ...
  const [isEditPage, setIsEditPage] = useState(
    window.location.pathname.includes('profile_edit')
  )
  return (
    <>
        ...
        {loggedInUser ? (
          <>
            <div className='flex flex-row items-end flex-wrap-reverse gap-3 mt-3'>
              ...
              <div
                id='title'
                className='order-1 min-w-1/2 w-fit rounded-md px-3 py-1 box-colors flex items-center space-x-2'
              >
                <div className='text-sm xs:text-md sm:text-lg/relaxed font-bold dark:text-text-dark hover:text-dark-amethyst-smoke-600 dark:hover:text-amethyst-smoke-600 duration-200'>
                ...
                {!isEditPage && (
                  <Link to='/profile/profile_edit'>
                    ...
                    <Edit
                      size={18}
                      className='hover:text-dark-amethyst-smoke-600 dark:hover:text-amethyst-smoke-600 duration-200'
                    />
                  </Link>
                )}
              </div>
            </div>
            {/* <div id='account_verify_status'>
              {!loggedInUser?.emailVerification&& (
                <div className='text-xs xs:text-sm capitalize'>
