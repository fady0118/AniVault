...
export default function ExtraFilters ({ registerCollectors, view }) {
  ...
  function calcDim () {
    ...
    if (width + btnRect.left <= contRect.right) {
      ...
      extraFiltersRef.current.style.top = `${
        contRect.bottom - contRect.top + 2
      }px`
    } else {
      ...
      {!view ? (
        ...
        <div id='Filters' className=''>
          <label className='group peer w-full p-1 flex flex-row items-center justify-between rounded-md box-colors-stronger hover:cursor-pointer hover:brightness-110 duration-200'>
            ...
            <Settings2
              onClick={calcDim}
              ref={extraFiltersBtnRef}
              size={16}
              className='stroke-text-light-70 dark:stroke-text-dark-70 group-hover:stroke-text-light dark:group-hover:stroke-text-dark'
            />
          </label>
          <div
            ref={extraFiltersRef}
            className={`absolute hidden peer-has-checked:grid rounded-md box-colors-stronger grid-cols-3 gap-1 p-2 `}
          >
            <ScoreFilter
              registerCollector={registerCollectors.scoreCollector}
            />
            <DateFilter
              data={dateTypesData}
              registerCollector={registerCollectors.dateCollector}
            />
          </div>
        </div>
      ) : (
