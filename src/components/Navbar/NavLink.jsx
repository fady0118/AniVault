...
export default function NavLink ({ classes, LinkTitle, data, ref }) {
  const { loggedInUser, userData, avatarImg, logout } = useAuth()
  ...
  function calcPos (e) {
    ...
    if (navLinkRect.left + navLinkBoxRect.width > navBarRect.right) {
      ...
      navLinkBoxRef.current.style.left = `${
        navLinkRect.left - navBarRect.left
      }px`
    ...
    <div className=' group wrapperLink flex flex-row items-center'>
      {data.length ? (
        ...
        <>
          ...
          <div
            onMouseEnter={e => calcPos(e)}
            className='w-6 aspect-square relative wrapper inline-block overflow-hidden duration-200'
          >
              <img
              ...
              {!loggedInUser?.emailVerification && (
                <div className='absolute bottom-0 right-0 w-2 h-2 rounded-full bg-rose-600 dark:bg-rose-500'></div>
              )}
          </div>
          {/* logout largeScreens form */}
          <div
            ref={navLinkBoxRef}
            className='w-fit absolute box-colors-darker border border-dark-amethyst-smoke-50/10 dark:border-amethyst-smoke-50/10 rounded-md linkTarget'
          >
            <div className='w-full h-full p-2 flex flex-col items-start gap-y-1'>
              <div className='w-full flex flex-col gap-y-1'>
                ...
                <div className='w-fit grid grid-cols-1 gap-1.5'>
                  <Link
                    className='text-nowrap opacity-65 hover:cursor-pointer hover:opacity-100 duration-200'
                    to='/profile'
                  >
                    Your Account
                  </Link>
