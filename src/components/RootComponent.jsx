...
export default function RootComponent({ Root, filterData, genresData, sortData }) {
  ...
  function handleApplyFilter() {
    ...
    let rating;
    if (Root === "anime") rating = collectorStore.current.rating();
    setSearchParams(
      Root === "anime"
        ? {
            type,
      ...
      <div className="relative left-1/2 -translate-x-1/2 z-10 w-full flex justify-center space-y-3 pt-15 pb-3">
        <div className="w-[95vw] flex flex-col ">
          ...
          <div className="order-2 px-3 py-1">
            {windowWidth > 600 ? (
              <>
                <div id="disclaimer" className="w-full mb-2 flex flex-row items-center gap-x-2 py-1.5 px-2.5 box-colors-stronger border border-indigo-600/60 rounded-r-md text-3xs xs:text-2xs">
                ...
                <div id="header" className="z-30 relative w-full flex flex-row items-center justify-between sm:justify-start sm:gap-x-4 capitalize text-2xs font-light">
          ...
          <AnimeMangaContainer searchParams={effectiveSearchParams} itemType={Root} />
        </div>
