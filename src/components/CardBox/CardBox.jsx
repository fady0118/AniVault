...
const classes = {
  ...
  role_class:
    'absolute capitalize top-0 right-0 bg-amethyst-smoke-200/70 text-text-light dark:bg-dark-amethyst-smoke-200/70 dark:text-text-dark',
  ...
  return (
    <div className='overflow-x-scroll no-scrollbar flex mt-2 mx-2 space-x-0.5'>
      {transformedDataArr.slice(0, 10).map((dataEntry, i) => (
        <div
          key={i}
          className={`flex flex-col ${widthClass[num]} min-w-18 shrink-0`}
        >
          <Box
            dataObj={{ data: dataEntry.data1 }}
            classes={classes}
            image_class={image_class}
          />
          <Box
            dataObj={{ data: dataEntry.data2 }}
            classes={classes}
            image_class={image_class}
          />
        </div>
