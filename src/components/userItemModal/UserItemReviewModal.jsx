      ...
      <div className="w-full grid grid-cols-1 xs:grid-cols-5 gap-x-4 gap-y-2 min-h-36">
        ...
        >
          ...
          <TextAreaToolBox
            metaData={{
              title: "Review Text",
              placeholder: `Share your thoughts on this ${mediaType || "item"}...`,
            }}
            ...
            <div className="flex gap-x-1 flex-nowrap w-full max-w-xs py-2">
              {Array.from({ length: 10 }, (_, i) => i + 1).map((i) => (
                <button
                  ...
                  aria-label={`Rate ${i}`}
                  className={`@container w-1/10 aspect-square flex justify-center items-center rounded-sm border duration-150 cursor-pointer px-2 py-1 ${rating === i ? "bg-amethyst-smoke-800 text-white border-amethyst-smoke-700" : "bg-transparent border-amethyst-smoke-800/20 hover:border-indigo-500"}`}
                >
