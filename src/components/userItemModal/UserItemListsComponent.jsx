...
export default function UserItemListsComponent ({
  aniListData,
  mediaType,
  userItemTableData
}) {
    ...
    try {
      const res = await tablesDB.listRows({
        ...
        queries: [
          ...
          Query.select(['*', 'listItem_id.*'])
        ]
      })
      console.log(res?.rows)
      if (res?.rows?.length) {
        const flattenedLists = Object.fromEntries(
          (res?.rows ?? []).map(({ name, $id, listItem_id, is_public }) => [
            $id,
            {
              name,
              listItems: listItem_id.map(i => Number(i.aniList_id)),
              is_public
            }
          ])
        )
  ...
  async function updateLists () {
    try {
      setStatus('loading')
      await Promise.all(
        Object.entries(selectedLists).map(
          ([listId, { notes, is_public, is_item_public }]) =>
            addItemToList(
              aniListData?.aniList_id ?? userItemTableData?.aniList_id,
              aniListData?.coverImage?.large ??
                aniListData?.coverImage?.medium ??
                userItemTableData?.cached_img,
              aniListData?.title ?? userItemTableData?.title,
              mediaType,
              notes,
              listId,
              is_public,
      ...
      if (newList?.name) {
        const newListRes = await createNewList(
          newList?.name,
          null,
          loggedInUser.$id,
          newList?.is_public
  ...
  useEffect(() => {
    if (!Object.keys(selectedLists).length && !newList?.name) return
    setStatus('modified')
        ...
        <div className='flex flex-row flex-wrap items-center justify-between gap-2'>
          <div>
            <p className='text-sm font-semibold'>
              Add {aniListData?.title || userItemData?.title} to one of your
              custom lists
            </p>
            <p className='text-xs text-amethyst-smoke-800 dark:text-amethyst-smoke-600'>
          ...
          <section className='rounded-2xl border border-white/10 p-3 bg-transparent'>
            <div className='mb-2 flex items-center justify-between gap-2'>
              <div>
                ...
                <p className='text-xs text-amethyst-smoke-800 dark:text-amethyst-smoke-600'>
                  Select one or more lists to add this item.
                </p>
              </div>
            </div>
            <div className='grid gap-2 max-h-52 overflow-y-auto pr-2'>
              {userListsData?.flattenedLists ? (
                Object.entries(userListsData?.flattenedLists).map(
                  ([listId, { name, is_public, listItems }]) => {
                    const alreadyAdded = listItems?.includes(
                    ...
                    return (
                      <label
                        key={listId}
                        htmlFor={`${listId}-list`}
                        className={`group grid rounded-2xl border p-2.5 transition ${
                          alreadyAdded
                            ? 'border-emerald-500/40 bg-emerald-500/10 dark:bg-emerald-500/5'
                            : selected
                            ? 'border-primary/40 bg-primary/10 dark:bg-primary/5 hover:cursor-pointer'
                            : 'border-amethyst-smoke-800/20 dark:border-amethyst-smoke-600/20 bg-transparent hover:cursor-pointer hover:border-primary/50 hover:bg-primary-900/10'
                        }`}
                      >
                        <div className='flex items-start justify-between'>
                          <div className='flex flex-row items-end gap-1.5 text-xs'>
                            <p className='truncate font-medium text-amethyst-smoke-950 dark:text-amethyst-smoke-100'>
                              {name}
                            </p>
                            <p className='text-[0.85em] text-amethyst-smoke-800 dark:text-amethyst-smoke-600'>
                              {is_public ? 'Public list' : 'Private list'}
                            </p>
                          </div>
                          <input
                            id={`${listId}-list`}
                        ...
                        {alreadyAdded ? (
                          <p className='text-2xs text-emerald-600 dark:text-emerald-300'>
                            Already contains this item.
                          </p>
                        ) : (
                          <p className='text-2xs text-amethyst-smoke-800 dark:text-amethyst-smoke-600'>
                            Tap to add this item to the list.
                          </p>
                        )}
                      </label>
                    )
          ...
          <section className='rounded-2xl border border-amethyst-smoke-100/10 p-3 bg-transparent'>
            <div className='flex flex-row items-center justify-between gap-2 flex-wrap'>
              ...
              <div className='flex items-center text-amethyst-smoke-400 gap-2 text-xs'>
                <p className='text-[0.9em] text-amethyst-smoke-800 dark:text-amethyst-smoke-600'>
                  List privacy
                </p>
                <label className='flex cursor-pointer items-center gap-2 rounded-full bg-transparent text-amethyst-smoke-900 dark:text-amethyst-smoke-500'>
                  <span className='text-xs'>
                    {newList?.is_public ? 'Public' : 'Private'}
                  </span>
                  <input
                    type='checkbox'
              ...
              {newList?.name || Object.keys(selectedLists).length ? (
                <>
                  <div className='grid gap-3'>
                    {Object.entries(selectedLists).map(
                      ([listId, { name, notes, is_item_public }]) => (
                        <div
                          key={listId}
                          className='rounded-2xl border border-amethyst-smoke-100/10 section-colors-medium p-3'
                        >
                          <div className='flex flex-row items-center justify-between gap-1 flex-wrap'>
                            <p className='text-sm font-medium text-amethyst-smoke-950 dark:text-amethyst-smoke-100'>
                              {name}
                            </p>
                            <div className='flex items-center text-amethyst-smoke-400 gap-2 text-xs'>
                              <p className='text-[0.9em] text-amethyst-smoke-800 dark:text-amethyst-smoke-600'>
                      ...
                      <div className='rounded-2xl border border-amethyst-smoke-100/10 section-colors-medium p-3'>
                        <div className='flex flex-row items-center justify-between gap-1 flex-wrap'>
                          ...
                          <div className='flex items-center text-amethyst-smoke-400 gap-2 text-xs'>
                            ...
                            <label className='flex cursor-pointer items-center gap-2 rounded-full bg-transparent text-amethyst-smoke-900 dark:text-amethyst-smoke-500'>
                              <span className='w-10'>
                                {newList?.is_item_public ? 'Public' : 'Private'}
                              </span>
                              <input
                                className='toggle toggle-primary toggle-xs bg-transparent not-checked:text-amethyst-smoke-600'
                                type='checkbox'
              ...
              {status === 'loading' ? (
                ...
                <>
                  {status === 'success' ? (
                    <div
                      role='alert'
                      className='text-emerald-600 dark:text-emerald-400 rounded-sm px-1 py-0.5 text-xs'
                    >
                    ...
                    <>
                      {error ? (
                        <div
                          role='alert'
                          className='text-rose-600 dark:text-rose-400 rounded-sm px-1 py-0.5 text-xs'
                        >
                          <span>{error}</span>
                        </div>
                      ) : null}
