...
export default function HomeSlider ({ season }) {
  ...
  function scrollIntoView (index) {
    ...
    slideNodes[((index-1)<0?season.length-1:index-1)%season.length].querySelector('img.img').classList.remove('animate-slide')
  ...
  return (
    <>
      <div className='w-fit h-fit'>
        <div
          id='slider'
          className='relative w-full rounded-xl left-1/2 -translate-x-1/2 h-screen flex overflow-y-hidden overflow-x-scroll no-scrollbar pointer-events-auto'
          ref={sliderRef}
        >
          {season.map((animeData, i) => (
            <Slide key={i} index={i} animeData={animeData} openModal={openModal} />
          ))}
        </div>
      ...
      {showVideoModal && videoRef.current && (
        <VideoModal closeModal={closeModal} link={videoRef.current} />
      )}
    </>
  )
}
