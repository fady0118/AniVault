...
export default function RecentManga () {
    ...
    queries: [
      {
        ...
        queryFn: async () => {
          const res = await jikanFetch(
            `https://api.jikan.moe/v4/manga?type=manga&sfw=true&genres_exclude=9,12,49&status=publishing&order_by=start_date&sort=desc&page=${
              mangaCurrentPage || 1
            }`
          )
          const recentMangaData = await res.json()
          const uniqueMangaData = [
            ...new Map(
              recentMangaData.data.map(item => [item.mal_id, item])
            ).values()
          ]
      ...
      },
      {
        ...
        queryFn: async () => {
          const res = await jikanFetch(
            `https://api.jikan.moe/v4/manga?type=novel&sfw=true&genres_exclude=9,12,49&status=publishing&order_by=start_date&sort=desc&page=${
              novelCurrentPage || 1
            }`
          )
          const recentNovelData = await res.json()
          const uniqueNovelData = [
            ...new Map(
              recentNovelData.data.map(item => [item.mal_id, item])
            ).values()
          ]
      ...
      },
      {
        ...
        queryFn: async () => {
          const res = await jikanFetch(
            `https://api.jikan.moe/v4/manga?type=manhwa&sfw=true&genres_exclude=9,12,49&status=publishing&order_by=start_date&sort=desc&page=${
              manhwaCurrentPage || 1
            }`
          )
          const recentManhwaData = await res.json()
          const uniqueManhwaData = [
            ...new Map(
              recentManhwaData.data.map(item => [item.mal_id, item])
            ).values()
          ]
  ...
  function handleInfoShow (e) {
    ...
    if (rect.x + rect.width >= window.innerWidth - 240)
      rect.x = window.innerWidth - rect.width - 260
    ...
    if (recent === 'manga') {
      popupDataRef.current = recentMangaQ.data.data.find(
        i => i.mal_id === Number(e.target.parentElement.dataset.malId)
      )
    } else if (recent === 'novel') {
      popupDataRef.current = recentNovelQ.data.data.find(
        i => i.mal_id === Number(e.target.parentElement.dataset.malId)
      )
    } else if (recent === 'manhwa') {
      popupDataRef.current = recentManhwaQ.data.data.find(
        i => i.mal_id === Number(e.target.parentElement.dataset.malId)
      )
    }
    // data
  ...
  function renderPopupData () {
    ...
    //rating, score
    element.querySelector('#rating>p').textContent =
      popupDataRef?.current?.rating
    element.querySelector('#score>p').textContent =
      popupDataRef?.current?.score || '?'
    // synopsis
    element.querySelector('#synopsis>p').textContent =
      popupDataRef?.current?.synopsis || 'no desciption found.'
    // aired, status
    element.querySelectorAll('#published>p')[1].textContent =
      popupDataRef?.current?.published?.string
    element.querySelectorAll('#status>p')[1].textContent =
      popupDataRef?.current?.status
    // genres
    const genresDiv = element.querySelector('#genres>div')
    genresDiv.innerHTML = popupDataRef?.current?.genres?.reduce(
      (c, genre) =>
        c +
        ` <p class="font-medium text-[0.9em] m-0.5 px-1 rounded-xl border subtle-border-colors">${genre.name}</p>`,
      ''
    )
    // hyperlink
    if (popupDataRef?.current?.mal_id) {
      element.querySelector(
        '#details'
      ).href = `/manga/${popupDataRef?.current?.mal_id}`
    }
  }
  ...
  function swapPage (type) {
    if (recent === 'manga') {
      if (type === 'next') {
        setMangaCurrentPage(s =>
          recentMangaQ?.data?.pagination?.has_next_page ? s + 1 : s
        )
      } else {
      ...
      if (type === 'next') {
        setNovelCurrentPage(s =>
          recentNovelQ?.data?.pagination?.has_next_page ? s + 1 : s
        )
      } else {
      ...
      if (type === 'next') {
        setManhwaCurrentPage(s =>
          recentManhwaQ?.data?.pagination?.has_next_page ? s + 1 : s
        )
      } else {
      ...
      <div className='w-full flex flex-row flex-wrap justify-between items-center text-md/relaxed sm:text-xl/relaxed font-extrabold uppercase'>
        ...
        <div className='flex flex-row flex-wrap items-center gap-x-5'>
          ...
          >
            ...
            {recent === 'manga' ? (
              <div>{mangaCurrentPage}</div>
            ) : recent === 'novel' ? (
              <div>{novelCurrentPage}</div>
            ) : (
              <div>{manhwaCurrentPage}</div>
            )}
              ...
              <>
                {recentMangaQ?.data?.uniqueMangaData?.map((item, i) => (
                  ...
                  >
                    ...
                    >
                      <Info
                        className='stroke-indigo-500 dark:stroke-indigo-400 pointer-events-none'
                        size={20}
                      />
                    </div>
                  </div>
              ...
              <>
                {recentNovelQ?.data?.uniqueNovelData?.map((item, i) => (
                  ...
                  >
                    ...
                    >
                      <Info
                        className='stroke-pink-600 dark:stroke-pink-400 pointer-events-none'
                        size={20}
                      />
                    </div>
                  </div>
