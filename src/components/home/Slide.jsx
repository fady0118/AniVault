...
export default function Slide({ animeData, openModal }) {
  ...
  return (
    <>
      <div className={`relative slide w-full h-full flex justify-end items-center shrink-0 text-6xl overflow-hidden `}>
        <div className="w-full h-full absolute top-0 left-0 "></div>
        <div className={`absolute z-20 top-0 left-0 w-full h-full ${gradientStyles}`}>
          <div className="w-full sm:w-[65%] h-full flex flex-col justify-end items-start">
            <div className="w-full sm:w-4/5 md:w-2/3 box-border h-1/2 sm:h-2/3 flex flex-col space-y-3 sm:space-y-6 justify-center px-9">
              ...
              <div className="w-full text-xl sm:text-4xl font-extrabold">{title}</div>
              ...
              {animeData?.description && (
                <div 
                  id="synopsis" 
                  className="flex text-xs font-light max-h-1/6 max-lines-4 cutoff-text-abs line-clamp-4"
                ...
                <div className="flex justify-between text-2xs sm:text-xs w-[90%] px-5">
                  <div className="flex-col space-y-0.5">
                    ...
                    <div className="font-bold text-xs xs:text-md">
                      {animeData.averageScore ? `${animeData.averageScore}%` : "N/A"}
                    </div>
                  </div>
                  <div className="flex-col space-y-0.5">
                    <div className="font-extralight">Popularity</div>
                    <div className="font-bold text-xs xs:text-md">
                      {animeData.popularity ? `#${animeData.popularity.toLocaleString()}` : "N/A"}
                    </div>
                  </div>
                  <div className="flex-col space-y-0.5">
                ...
                <button
                  onClick={() => {
                    if (animeData.trailer?.site === "youtube") {
                      openModal(`https://www.youtube.com/embed/${animeData.trailer.id}`);
                    }
                  }}
                  disabled={!animeData.trailer}
                  className={`flex justify-center items-center space-x-2 w-3/5 2xs:w-2/5 aspect-9/2 rounded-lg uppercase text-xs 2xs:text-sm font-semibold border border-rose-600 text-rose-600 dark:border-emerald-600 dark:text-emerald-600 hover:cursor-pointer hover:brightness-125 hover:-translate-y-1 duration-300 ${!animeData.trailer ? 'opacity-50 hover:-translate-y-0 cursor-not-allowed' : ''}`}
                >
                  <Play size={18} />
                  <span>Trailer</span>
                </button>
